...
def _get_resolved_mentor(db: Session, mentor_id_or_user_id: str) -> Optional[Mentor]:
    """Helper to resolve Mentor record by either canonical Mentor.id or User.id."""
    return (
        db.query(Mentor)
        .filter((Mentor.id == mentor_id_or_user_id) | (Mentor.user_id == mentor_id_or_user_id))
        .first()
    )
...
def get_mentor_me(
    ...
    if not mentor:
        # Automatically provision mentor profile if missing
        mentor = Mentor(
    ...
    company_name = mentor.company_name or (mentor.company.name if mentor.company else "Independent / Partner")
...
def get_mentor_interns(
    ...
    for app in applications:
        ...
        internship_title = internship.title if internship else "Software Engineering Intern"
        company_name = (
            internship.company.name
            if internship and internship.company
            else (mentor.company_name or "CloudScale Distributed Systems")
        )
        ...
        total_reports = 12
        progress_pct = min(100, int((reports_count / total_reports) * 100))
        ...
        has_low_score = any(r.mentor_score and r.mentor_score < 3.5 for r in reports)
        has_revision = any(r.status == "Needs Revision" for r in reports)
        if has_low_score or has_revision:
            attention_status = "NEEDS_ATTENTION"
        elif reports_count < 3 and total_tasks > 0 and completed_tasks == 0:
        ...
        results.append(
            MentorInternItem(
                ...
                student_email=student.user.email if student.user else "student@university.edu",
                student_id_number=student.student_id_number or "STU-2026-8842",
                college=student.college or "School of Engineering",
                department=student.department or "Computer Science",
                internship_id=internship.id if internship else app.internship_id,
                ...
                latest_report_status=latest_report.status if latest_report else "Pending Submission",
            )
        )
...
def create_evaluation(
    ...
    student = (
        db.query(Student)
        .filter((Student.id == payload.student_id) | (Student.user_id == payload.student_id))
        .first()
    )
    if not student:
        raise HTTPException(
...
def list_tasks(
    ...
    tasks = (
        db.query(Task)
        .filter((Task.mentor_id == resolved_id) | (Task.mentor_id.is_(None)))
